...
import static org.zuri.BStringMatches.matchesAs;
import static org.hamcrest.core.StringStartsWith.startsWith;

public class ZuriTestRunner extends ParentRunner<ZuriTestRunner.TestCase> {

  private static final String SOURCE_SUFFIX = ".zu";
  ...
  public ZuriTestRunner(Class<?> runningClass) throws InitializationError {
    super(runningClass);
    try {
  ...
  protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
    ZuriTestSuite suite = c.getAnnotation(ZuriTestSuite.class);
    if (suite == null) {
      throw new InitializationError(String.format(
        "@%s annotation required on class '%s' to run with '%s'.",
        ZuriTestSuite.class.getSimpleName(),
        c.getName(),
        ZuriTestRunner.class.getSimpleName()
      ));
    }
    ...
    if (suite.testCaseDirectory() != ZuriTestSuite.class) {
  ...
  private static void run(Context context, Path path, PrintWriter out) throws IOException {
    try {
      ...
      Source source = Source.newBuilder(ZuriLanguage.ID, path.toFile()).build();
...
//        ex.printStackTrace(out);
  ...
  public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
    ...
    ZuriTestRunner suite = new ZuriTestRunner(testClass);
    if (args.length > 0) {
