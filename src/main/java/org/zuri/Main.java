...
public class Main {
  ...
  public static void main(String[] args) throws IOException {
    ...
    try (
      Context defaultContext = Context.newBuilder(ZuriLanguage.ID)
        .in(System.in)
        .out(System.out)
        .err(System.err)
...
//        })
  ...
  private static Source getSource(String path) throws IOException {
    return Source.newBuilder(ZuriLanguage.ID, new File(path)).build();
  }

  private static Source createSource(String content) throws IOException {
    return Source.newBuilder(ZuriLanguage.ID, content, "<script>").build();
  }
...
//    if (group.equals("zuri") || key.equals("inspect")) {
