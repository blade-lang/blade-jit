...
import org.zuri.Main;
import org.zuri.language.nodes.common.NDynamicObjectRefNode;
import org.zuri.language.nodes.common.NGlobalScopeObjectNode;
import org.zuri.language.nodes.common.NGlobalScopeObjectNodeGen;
import org.zuri.language.nodes.NNode;
import org.zuri.language.nodes.expressions.*;
import org.zuri.language.nodes.expressions.arithemetic.*;
import org.zuri.language.nodes.expressions.bitwise.*;
import org.zuri.language.nodes.expressions.logical.*;
import org.zuri.language.nodes.functions.*;
import org.zuri.language.nodes.imports.NImportNode;
import org.zuri.language.nodes.list.NReadListIndexNodeGen;
import org.zuri.language.nodes.list.NWriteListIndexNodeGen;
import org.zuri.language.nodes.list.NListLiteralNode;
import org.zuri.language.nodes.literals.*;
import org.zuri.language.nodes.statements.*;
import org.zuri.language.nodes.statements.loops.*;
import org.zuri.language.nodes.string.NStringLiteralNode;
import org.zuri.language.nodes.using.NUsingNode;
import org.zuri.language.nodes.using.NWhenNode;
import org.zuri.language.parser.BaseVisitor;
import org.zuri.language.parser.Parser;
import org.zuri.language.parser.ast.AST;
import org.zuri.language.parser.ast.Expr;
...
import org.zuri.language.shared.LocalRefSlot;
...
public class ZuriTranslator extends BaseVisitor<NNode> {
  ...
  private ZuriClass currentClass = null;

  public ZuriTranslator(Parser parser, BuiltinClassesModel classesModel) {
    this.parser = parser;
    ...
    for (Map.Entry<String, ZuriClass> classEntry : classesModel.builtinClasses.entrySet()) {
  ...
  public NNode visitAssignExpr(Expr.Assign expr) {
    ...
    if (expr.expression instanceof Expr.Identifier identifier) {
      ...
      if (member == null) {
        ...
        if (member instanceof NFrameMember.FunctionArgument memberValue) {
          ...
          if (local.constant) {
            throw ZuriRuntimeError.error(value, "Assignment to constant variable '", name, "'");
          }
    ...
    throw ZuriRuntimeError.error(value, "Invalid assignment expression");
  }
  ...
  public NNode visitVarStmt(Stmt.Var stmt) {
    ...
    if (isConstant && stmt.value == null) {
      throw ZuriRuntimeError.error(value, "Constant '", name, "' not initialized");
    }
    ...
    if (state != ParserState.TOP_LEVEL) {
      ...
      if (localScope.peek().putIfAbsent(name, new NFrameMember.LocalVariable(slot, isConstant)) != null) {
        throw ZuriRuntimeError.error(value, "'", name, "' is already declared in this scope");
      }
  ...
  public NNode visitReturnStmt(Stmt.Return stmt) {
    ...
    if (state != ParserState.FUNC_DEF) {
      throw ZuriRuntimeError.error(value, "`return` keyword is not allowed in this scope");
    }
  ...
  public NNode visitClassStmt(Stmt.Class stmt) {
    if (state == ParserState.FUNC_DEF) {
      throw ZuriRuntimeError.create("Classes cannot be nested in functions");
    }
    ...
    if (frameMember instanceof NFrameMember.ClassObject classMember) {
      ZuriClass superClassObject = classMember.object;
      classObject = new ZuriClass(objectShape, className, superClassObject);
    } else {
      throw ZuriRuntimeError.create("Class '", className, "' extends unknown or frozen class '", superClass, "'");
    }
  ...
  public NNode visitSelfExpr(Expr.Self expr) {
    if (currentClass == null) {
      throw ZuriRuntimeError.create("`self` keyword not allowed outside a class");
    }
  ...
  public NNode visitParentExpr(Expr.Parent expr) {
    if (currentClass == null) {
      throw ZuriRuntimeError.create("`parent` keyword not allowed outside a class");
    }
  ...
  public NNode visitCatchStmt(Stmt.Catch stmt) {
    ...
    if (stmt.name != null) {
      ...
      if (localScope.peek().putIfAbsent(errorName, new NFrameMember.LocalVariable(slot, true)) != null) {
        throw ZuriRuntimeError.error(thenBody, "'", errorName, "' is already declared in this scope");
      }
  ...
  public NNode visitImportStmt(Stmt.Import stmt) {
    ...
    if(stmt.path.startsWith("_")) {
      ...
      if (stmt.path.startsWith(".")) {
        ...
        moduleFile = new File(currentDir, path + ".zu");
        if (!moduleFile.exists()) {
          moduleFile = new File(currentDir, String.join(sep, path, "index.zu"));
          if (!moduleFile.exists()) {
        ...
        // Non-relative imports start from the `.zuri/libs` directory in the current directory
        // and progress to the application root directory.
        moduleFile = new File(currentDir, String.join(sep, ".zuri", "libs", stmt.path + ".zu"));
        if (!moduleFile.exists()) {
          moduleFile = new File(currentDir, String.join(sep, ".zuri", "libs", stmt.path, "index.zu"));
          if (!moduleFile.exists()) {
            ...
            // Next progress to the `apps` directory of the Zuri installation.
            ...
            moduleFile = new File(rootDirectory, stmt.path + ".zu");
            if (!moduleFile.exists()) {
              moduleFile = new File(rootDirectory, String.join(sep, stmt.path, "index.zu"));
            }


            if (!moduleFile.exists()) {
              // If we still haven't found the module,
              // We can start checking the Zuri's standard library directory.
