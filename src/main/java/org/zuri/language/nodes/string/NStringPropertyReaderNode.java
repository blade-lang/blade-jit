package org.zuri.language.nodes.string;

import com.oracle.truffle.api.dsl.Cached;
import com.oracle.truffle.api.dsl.Fallback;
import com.oracle.truffle.api.dsl.ImportStatic;
import com.oracle.truffle.api.dsl.Specialization;
import com.oracle.truffle.api.interop.InteropLibrary;
import com.oracle.truffle.api.interop.UnknownIdentifierException;
import com.oracle.truffle.api.interop.UnsupportedMessageException;
import com.oracle.truffle.api.library.CachedLibrary;
import com.oracle.truffle.api.strings.TruffleString;
import org.zuri.language.nodes.NBaseNode;
import org.zuri.language.runtime.*;

@ImportStatic(ZString.class)
@SuppressWarnings("truffle-inlining")
public abstract class NStringPropertyReaderNode extends NBaseNode {
  public static final String LENGTH_PROP = "length";

  public static NStringPropertyReaderNode create() {
    return NStringPropertyReaderNodeGen.create();
  }

  public abstract Object executeProperty(TruffleString self, Object property);

  @Specialization
  protected Object readStringIndex(
    TruffleString string, long index,
    @Cached TruffleString.CodePointLengthNode lengthNode,
    @Cached TruffleString.SubstringNode substringNode
  ) {
    long stringLength = ZString.length(string, lengthNode);
    if (index < 0) index = index + stringLength;

    return index < 0 || index >= stringLength
      ? ZString.EMPTY
      : ZString.substring(string, (int) index, 1, substringNode);
  }

  @Fallback
  protected Object readOthers(
    TruffleString string, Object property,
    @Cached(value = "languageContext().objectsModel.stringObject", neverDefault = false) ZuriClass stringClass,
    @CachedLibrary(limit = "3") InteropLibrary interopLibrary
  ) {
    try {
      return interopLibrary.readMember(stringClass, ZString.tryToString(property));
    } catch (UnsupportedMessageException e) {
      throw ZuriRuntimeError.error(this, e.getMessage());
    } catch (UnknownIdentifierException e) {
      return ZuriNil.SINGLETON;
    }
  }
}
